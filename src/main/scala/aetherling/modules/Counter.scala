...
class Counter_T(val n: Int, val i: Int, val incr_amount: Int, val int_type: STInt, val delay: Int)
  ...
  if (int_type.signed) {
    val wrap = counter_value.asInstanceOf[SInt] === ((n+i-1)*incr_amount).S
    ...
    val wrap = counter_value.asInstanceOf[UInt] === ((n+i-1)*incr_amount).U
...
class Counter_TS(val no: Int, val io: Int, val ni: Int, val incr_amount: Int, val int_type: STInt, val delay: Int)
  ...
  counter_t.valid_up := valid_up
  counter_s.valid_up := valid_up

  if (int_type.signed) {
    for (i <- 0 to ni-1) {
      O(i) := counter_s.O(i).asInstanceOf[SInt] + counter_t.O.asInstanceOf[SInt]
    }
  } else {
    for (i <- 0 to ni-1) {
      O(i) := counter_s.O(i).asInstanceOf[UInt] + counter_t.O.asInstanceOf[UInt]
    }
  }
...
class Counter_TN(val ns: IndexedSeq[Int], val is: IndexedSeq[Int], val incr_amount: Int, val int_type: STInt, val delay: Int)
  ...
  //printf(p"Nested_counters.last: ${nested_counters.last}\n")
  //printf(p"Nested_counters.valid: ${nested_counters.valid}\n")
  //printf(p"enabled: ${enabled}\n")
  ...
  if (int_type.signed) {
    when (enabled && nested_counters.last) { counter_value.asInstanceOf[SInt] := 0.S }
      .elsewhen(enabled && nested_counters.valid) { counter_value.asInstanceOf[SInt] := counter_value.asInstanceOf[SInt] +
        incr_amount.S(int_type.width.W)}
  } else {
    when (enabled && nested_counters.last) { counter_value.asInstanceOf[UInt] := 0.U }
      .elsewhen(enabled && nested_counters.valid) {counter_value.asUInt := counter_value.asUInt + incr_amount.U(int_type.width.W)}
  }
